...
actual class PoolColorArray : JniType {
    companion object {
        ...
        fun fromRawMemory(pointer: Long): PoolColorArray {
            return PoolColorArray().apply { rawMemory = pointer }
        }
    }

    actual constructor() {
         rawMemory = godot_pool_color_array_new()
    }

    actual constructor(other: PoolColorArray) {
         rawMemory = godot_pool_color_array_new_copy(other)
    }

    actual constructor(other: GDArray) {
         rawMemory = godot_pool_color_array_new_with_array(other)
    }
